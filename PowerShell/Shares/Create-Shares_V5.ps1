...
if (Get-ChildItem -Path $FilePath | Where-Object {$_.Name -like "Server Knowledge Base" -or "Network Knowledge Base"}){
    ...
    # Create share
    New-SmbShare -name "Server Support Share" -Path "$Filelocation\$FileName" -ChangeAccess 'SleepyGeeks\Srv Support Edmonton' -FullAccess 'SleepyGeeks\Domain Admins'
    ...
    # Create share
    New-SmbShare -name "Network Support Share" -Path $Filelocation\$FileName -ChangeAccess 'SleepyGeeks\Net Support Edmonton' -FullAccess 'SleepyGeeks\Domain Admins'
    # Cmd prompt version of assigning a drive letter
    #runas /user:administrator net use Z: "\\TEST01\TestShare" /persistent
    # Powershell version of assigning drive letter
