...
# Customized to PROJ3000 settings
function New-CustomHyperV{
    ...
    param(
        ...
        [String] $VMPath = "C:\ClusterStorage\Volume1\HyperV",
    
        [parameter(Mandatory = $false)] #CHANGE THIS LATER!!!!!!!!!!!!!!!!!!
        [String] $SysVHDPath = "C:\ClusterStorage\Volume1\Templates\winsrv2022base.vhdx",
        ...
        [parameter(Mandatory = $false)] # Configured later
        [String] $VMSwitchName,
    ...
    if ($VMSwitchName) {
        $VMSwitchName = Get-VMSwitch -Name $VMSwitchName -ErrorAction SilentlyContinue
    }
    ...
    if ($existingAdapter) {
        ...
        Copy-Item -Path $SysVHDPath -Destination $($VMPath + "\" + $VMName + "\" + "Virtual Machines")
        Rename-Item -Path $($VMPath + "\" + $VMName + "\" + "Virtual Machines" + "\" + $OsDiskInfo.Name) -NewName $($VMName + $OsDiskInfo.Extension)
        #Rename-Item -Path $($VMPath + "\" + "Virtual Machines" + "\" + $VMName + "\" + $OsDiskInfo.Name) -NewName $($VMName + $OsDiskInfo.Extension)
    
        # Attach the VHD(x) to the VM
        Add-VMHardDiskDrive -VMName $VMName -Path $($VMPath + "\" + $VMName + "\" + $VMName + $OsDiskInfo.Extension)
